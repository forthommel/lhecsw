...
public:
  ...
  StatusCode initialize() override {
    if (const auto status = AlgTool::initialize(); !status.isSuccess())
      return status;
    // initialise all interfacing objects
    cepgen_ = std::make_unique<cepgen::Generator>();
    cross_section_ = std::make_unique<HepMC3::GenCrossSection>();

    // retrieve process definition to feed CepGen something to generate
    ...
    // list of event modification algorithms to be run in sequence after CepGen event generation
    ...
    // list of secondary output modules to be initialised along the HepMC interface used in this module
    // (it allows, e.g. to monitor events production along their interfacing to LHeCSW)
    ...
    const auto xsec = cepgen_->computeXsection();  // initialisation finishes with the initial cross-section integration
    cross_section_->set_cross_section(xsec, xsec.uncertainty());  // everything is expressed in pb
    return StatusCode::SUCCESS;
  }
  inline StatusCode finalize() override {
    cepgen_.release();
    cross_section_.release();
    return StatusCode::SUCCESS;
  }
  inline StatusCode getNextEvent(HepMC3::GenEvent& hepmc_evt) override {
    hepmc_evt = HepMC3::CepGenEvent(cepgen_->next());
    hepmc_evt.set_cross_section(cross_section_);
    hepmc_evt.weights().push_back(1.);  // unweighted events are generated by default in CepGen
    return StatusCode::SUCCESS;
  }

private:
  std::unique_ptr<cepgen::Generator> cepgen_;               ///< main event generator instance
  std::shared_ptr<HepMC3::GenCrossSection> cross_section_;  ///< total cross-section container
  ...
  Gaudi::Property<std::string> modifiers_str_;  ///< list of event modification algos called sequentially for each event
  Gaudi::Property<std::string> outputs_str_;  ///< sequential list of event output modules called at each event generation
};
